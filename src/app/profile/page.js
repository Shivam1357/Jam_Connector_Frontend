// src/app/profile/page.js - Complete updated version
'use client'
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Load user profile data with address
        const mockUserData = {
          name: user?.name || 'John Doe',
          email: user?.email || 'john@example.com',
          phoneNumber: '+1234567890',
          profilePictureUrl: '',
          bio: 'Passionate guitarist with 10 years of experience',
  ...
  return (
    <div className="bg-gradient-to-br from-[#0a0a1a] via-[#1a0a2a] to-[#2a0a3a] text-white min-h-screen py-8 px-5 font-sans relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto">
        ...
        {/* Profile Summary */}
        <div className="mt-8 bg-white/5 p-6 rounded-2xl backdrop-blur-sm border border-white/10">
          <h4 className="text-lg font-semibold mb-4 text-gray-300">Profile Summary</h4>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
            <div className="text-center p-3 bg-white/10 rounded-xl">
              <div className="text-orange-400 font-semibold">Role</div>
              <div className="capitalize">{profileData.role}</div>
            </div>
            <div className="text-center p-3 bg-white/10 rounded-xl">
              <div className="text-cyan-400 font-semibold">Genres</div>
              <div>{profileData.selectedGenres.length} selected</div>
            </div>
            {profileData.role === 'MUSICIAN' && (
              <div className="text-center p-3 bg-white/10 rounded-xl">
                <div className="text-yellow-400 font-semibold">Instruments</div>
                <div>{profileData.selectedInstruments.length} selected</div>
              </div>
            )}
            <div className="text-center p-3 bg-white/10 rounded-xl">
              <div className="text-blue-400 font-semibold">Address</div>
              <div>{profileData.address.label || 'Not set'}</div>
            </div>
          </div>
        </div>
